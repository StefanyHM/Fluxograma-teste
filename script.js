...
var colourArray = ['#ffffff', '#fbc622', '#ff1f00', '#fab372', 'ffff88'];
// #fbc622: amarelo
		...
		if(value["requisito"].length)
		{
			for(let req of value["requisito"])
			{
				...
				if(!clone)
				{
					...
					clone.setAttribute("style", "stroke: #b44c10; stroke-width: 1; opacity = 1"); //era #ffffff
...
function boxLeave()
{
	if(boxFocused === undefined)
	{
		...
		for(let a = 0; a < arrows.length; a++)
		{
			...
			arrows[a].style.stroke = "#b44c10"; /*era #ffffff*/
